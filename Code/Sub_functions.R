...
#This script contains functions that are part of main_function.R
...
BayesKAT_MCMC<-function(inputAddress_y,inputAddress_X,inputAddress_Z,prior_H1=0.5,output_address){
  ...
  if (!require(Matrix)) {
    ...
    stop("stringr package not installed")
  }else if(!require(MASS)){
    
    stop("MASS package not installed")
  }else if(!require(matrixcalc)){
    
    stop("matrixcalc package not installed")
  }else if(!require(LaplacesDemon)){
    
    stop("LaplacesDemon package not installed")
  }else if(!require(mvtnorm)){
    
    stop("mvtnorm package not installed")
  }else if(!require(ttutils)){
    
    stop("ttutils package not installed")
  }else if(!require(BayesianTools)){
    
    stop("BayesianTools package not installed")
  }else if(!require(invgamma)){
    
    stop("invgamma package not installed")
  }
  else {
...
BayesKAT_MAP<-function(inputAddress_y,inputAddress_X,inputAddress_Z,prior_H1=0.5,output_address){
  ...
  }else if (!require(numDeriv)) {
    
    stop("numDeriv package not installed")
    
  }else if (!require(minqa)) {
    
    stop("minqa package not installed")
